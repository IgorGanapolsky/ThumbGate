...
test('YouTube landing steals FORMAT not the summit product', () => {
  ...
  assert.match(html, /\/go\/github\?/);
  assert.match(html, /\/go\/npm\?/);
  assert.match(html, /\/go\/marketplace\?/);
  assert.match(html, /cta_id=yt_github_hero/);
  assert.match(html, /cta_id=yt_npm_hero/);
  assert.match(html, /cta_id=yt_marketplace_hero/);
  assert.match(html, /What does it cost to start\?/);
  assert.match(html, /does not sell a paid diagnostic/);
  assert.doesNotMatch(html, /href="https:\/\/www\.npmjs\.com\/package\/thumbgate/);
  assert.doesNotMatch(html, /href="https:\/\/github\.com\/marketplace\/actions\/thumbgate-agent-governance/);
  assert.doesNotMatch(html, /href="https:\/\/github\.com\/IgorGanapolsky\/ThumbGate\?utm/);

...
test('seeded eval parser routes PDF to the async production parser', () => {
  const r = parseDocument({ type: 'pdf', content: '%PDF-1.4 binary' });
  assert.equal(r.ok, false);
  assert.equal(r.errors[0].code, 'async_parser_required');
  assert.match(r.errors[0].message, /importDocumentAsync/);
});

...
test('renderPlanMarkdown names all five GSD stages and page briefs', () => {
  ...
  assert.match(markdown, /OpenCode Guardrails \| Stop Repeated Agent Mistakes with ThumbGate/);
...
test('OpenCode guardrails page is discoverable and commercially classified', () => {
  const page = findSeoPageByPath('/guides/opencode-cli-guardrails');
  const sitemapEntry = THUMBGATE_SEO_SITEMAP_ENTRIES.find((entry) => entry.path === '/guides/opencode-cli-guardrails');

  assert.ok(page);
  assert.equal(page.query, 'opencode cli guardrails');
  assert.equal(page.pageType, 'integration');
...
test('writePlanOutputs persists machine-readable GSD artifacts', () => {
  ...
  try {
    ...
    assert.ok(pages.some((page) => page.path === '/guides/opencode-cli-guardrails'));

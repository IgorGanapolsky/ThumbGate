...
def test_rule_knapsack_selection():
    ...
    assert sum(by_id[i]["eval_time_ms"] for i in selected) <= 30.0
    assert sum(by_id[i]["token_footprint"] for i in selected) <= 450
    if GUROBI_AVAILABLE:
...
def test_load_input_rejects_filesystem_paths():
    with pytest.raises(ValueError, match="filesystem paths are not accepted"):
        load_input_payload("/etc/passwd")
    with pytest.raises(ValueError, match="filesystem paths are not accepted"):
        load_input_payload("../secrets.json")
...
def test_main_cli_rejects_path(capsys):
    code = main(["--mode", "routing", "--input", "/tmp/gurobi-escape.json"])
    body = json.loads(capsys.readouterr().out)
    assert code == 1

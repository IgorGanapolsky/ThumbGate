...
const inlineMarkPath = path.join(repoRoot, 'public', 'assets', 'brand', 'thumbgate-mark-inline-v4.svg');
...
test('ThumbGate inline mark SVG exists with transparent backdrop for header use', () => {
  assert.equal(
    ...
    true,
    'public/assets/brand/thumbgate-mark-inline-v4.svg must exist for header use',
  );
...
test('landing page header uses the inline (transparent) mark, not the app-icon tile', () => {
  ...
  assert.match(
    indexHtml,
    /<a[^>]*class="nav-logo"[^>]*>\s*<img[^>]*src="\/assets\/brand\/thumbgate-mark-inline-v4\.svg"/,
  );
  ...
  assert.doesNotMatch(
    ...
    'header must use thumbgate-mark-inline-v4.svg; the app-icon thumbgate-mark.svg renders as a tiny tile inline',
  );
});
...
test('dashboard, lessons, pro, and learn pages use the inline mark in their header', () => {
  ...
  for (const relPath of files) {
    ...
    assert.match(
      html,
      /src="\/assets\/brand\/thumbgate-mark-inline-v4\.svg"/,
      `${relPath} header must reference /assets/brand/thumbgate-mark-inline-v4.svg`,
    );
    assert.doesNotMatch(
      html,
      /src="\/brand\/thumbgate-mark\.svg"/,
...
test('checkout success (Context Gateway Activated) page renders the inline mark in its header', () => {
  ...
  assert.match(
    server,
    /class="brand-header"[^>]*>\s*<img[^>]*src="\/assets\/brand\/thumbgate-mark-inline-v4\.svg"/,
  );
  ...
  assert.doesNotMatch(
    ...
    'success page header must use thumbgate-mark-inline-v4.svg, not the app-icon tile variant',
  );
});
...
test('SEO-GSD generated page template uses the inline mark, not the app-icon tile', () => {
  ...
  assert.match(
    seoGsd,
    /class="brand"[^>]*>\s*<img[^>]*src="\/assets\/brand\/thumbgate-mark-inline-v4\.svg"/,
  );

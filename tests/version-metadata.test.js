...
test('hosted origin and repository metadata stay canonical across live-facing artifacts', () => {
  ...
  assert.match(publicLanding, /Claude Code/i);
  assert.match(publicLanding, /PreToolUse rule/i);
  assert.match(publicLanding, /recognizable tool calls and repeated patterns/i);
  assert.match(publicLanding, /https:\/\/github\.com\/IgorGanapolsky\/ThumbGate/);
  assert.match(publicLanding, /\/guide\?utm_source=website&utm_medium=hero/);
  assert.match(publicLanding, /\/checkout\/pro\?confirm=1&utm_source=website&utm_medium=hero/);
  assert.doesNotMatch(publicLanding, /href="\/go\/install\?/);
  assert.doesNotMatch(publicLanding, /href="\/go\/pro\?/);

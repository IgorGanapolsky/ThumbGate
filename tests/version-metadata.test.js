...
test('public docs render the current package version', () => {
  ...
  assert.match(chatgptInstall, /30-second regular-user flow/);
  assert.match(chatgptInstall, /Regular-user prompts/);
  assert.match(chatgptInstall, /Pre-action gate flow/);
  assert.match(chatgptInstall, /replying with 👍\/👎 or "thumbs up\/down"/);
  ...
  assert.match(chatgptInstall, /👎 this answer was too vague/i);
  assert.match(chatgptInstall, /Ask me anything\. After my answer, reply 👍/);
  ...
  assert.match(gptStoreSubmission, /Thumbs up\/down memory for ChatGPT answers/);
  assert.match(gptStoreSubmission, /Reply with thumbs up\/down\. I remember the lesson\./);
  assert.match(gptStoreSubmission, /pre-action gate for proposed AI-agent actions/);
  assert.match(gptStoreSubmission, /POST \/v1\/decisions\/evaluate/);
  assert.match(gptStoreSubmission, /Answer memory mode/);
  assert.match(gptStoreSubmission, /Pre-action gate mode/);
  assert.match(gptStoreSubmission, /Do not describe plain answer feedback as hard enforcement/);
  assert.match(gptStoreSubmission, /native rating buttons automatically save ThumbGate lessons/i);
  assert.match(gptStoreSubmission, /Five-star user experience rules/);
  assert.match(gptStoreSubmission, /Do not make the user write JSON/);
  ...
  assert.match(gptStoreSubmission, /👍 this format worked/i);

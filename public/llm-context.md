...
**Dirac comparison boundary**: Dirac is a token-efficient coding agent (hash-anchored edits, AST batching, no MCP). ThumbGate is the coding-agent PreToolUse firewall. Adjacent, not a substitute. Dirac's 64.8% cheaper public eval is theirs, not a ThumbGate measurement. Token efficiency is not an allow verdict. See `/compare/dirac`.

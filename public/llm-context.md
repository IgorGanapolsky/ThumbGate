...
**FAR.AI comparison boundary**: FAR.AI is an upstream nonprofit research institute focused on frontier AI safety, including red-teaming and evaluation. ThumbGate is a downstream runtime control layer that turns known operational failures into feedback records, reviewable prevention rules, pre-action verdicts, and audit evidence. They may overlap in broad AI-safety attention, but FAR.AI research and ThumbGate tool-call enforcement serve different workflow stages. See `/compare/far-ai`.

**Agoragentic comparison boundary**: Agoragentic is a governed agent OS plus marketplace (Triptych OS, mandate/budget/approvals, receipts). ThumbGate is the coding-agent PreToolUse firewall. Adjacent, not a substitute. A receipt is not independent world-state proof; a public listing is not invocable. See `/compare/agoragentic` and `/public-proof.json`.

**Dirac comparison boundary**: Dirac is a token-efficient Cline-fork coding agent (hash-anchored edits, AST batching, no MCP, approval plus `dirac -y` yolo). ThumbGate is the PreToolUse firewall for Claude Code, Cursor, and Codex. Adjacent SKU, overlapping buyer. Dirac's 64.8% cheaper public eval is theirs. There is no `thumbgate init --agent dirac`; Dirac native tools are not intercepted. See `/compare/dirac`.
...
- **Managed AI Agent Workflow Gate**: $499 one-time for one supported workflow. Includes a 60-minute working review, one configured local gate, its regression test, and written rollout/rollback proof within two business days after access and agreed materials are available.
- **Free local CLI**: Available for hands-on evaluation. It is an adoption path, not a second promoted paid offer.
...
- **Commercial boundary**: Unsupported workflows are not retained as paid work. Legacy Pro, sprint, and enterprise rails are not promoted during this single-offer experiment.

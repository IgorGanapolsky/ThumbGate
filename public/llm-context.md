...
**Financial Transaction Control**: Detected purchases, paid plan changes, transfers, payouts, refunds, and similar economic mutations fail closed unless they carry an explicit positive USD budget and an exact-action requisition approved by an independently authenticated human reviewer. Reservations are short-lived, requester-bound, single-use, consumed only at the final allow boundary, and settled against receipt evidence. A `$0` budget is a deterministic never-spend mode. This is an ERP-style interlock, not a bank or refund service: provider-side card limits and settlement controls remain separate.

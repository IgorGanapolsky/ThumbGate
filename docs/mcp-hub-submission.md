...
```bash
claude mcp add rlhf -- npx -y mcp-memory-gateway@0.8.9 serve
```
...
```json
{
  "mcpServers": {
    "rlhf": {
      "command": "npx",
      "args": ["-y", "mcp-memory-gateway@0.8.9", "serve"],
      "env": {
...
- **stdio** (primary): `npx -y mcp-memory-gateway@0.8.9 serve` — version-pinned portable MCP launcher for Claude Code desktop and CLI

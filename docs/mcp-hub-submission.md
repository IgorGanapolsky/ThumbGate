...
```bash
claude mcp add rlhf -- npx -y mcp-memory-gateway@0.9.2 serve
```
...
```json
{
  "mcpServers": {
    "rlhf": {
      "command": "npx",
      "args": ["-y", "mcp-memory-gateway@0.9.2", "serve"],
      "env": {
...
- **stdio** (primary): `npx -y mcp-memory-gateway@0.9.2 serve` — version-pinned portable MCP launcher for Claude Code desktop and CLI

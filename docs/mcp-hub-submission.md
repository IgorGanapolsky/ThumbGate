...
```bash
claude mcp add rlhf -- npx -y mcp-memory-gateway@0.8.7 serve
```
...
- **stdio** (primary): `npx -y mcp-memory-gateway@0.8.7 serve` — version-pinned portable MCP launcher for Claude Code desktop and CLI

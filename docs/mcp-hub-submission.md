...
```bash
claude mcp add thumbgate -- npx -y thumbgate@1.16.15 serve
```
...
```json
{
  "mcpServers": {
    "thumbgate": {
      "command": "npx",
      "args": ["-y", "thumbgate@1.16.15", "serve"],
      "env": {
...
- **stdio** (primary): `npx -y thumbgate@1.16.15 serve` — version-pinned portable MCP launcher for Claude Code desktop and CLI

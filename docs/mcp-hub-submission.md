...
```bash
claude mcp add rlhf -- npx -y mcp-memory-gateway@0.8.5 serve
```
...
Current self-serve commercial offer: Pro ($19/mo): https://rlhf-feedback-loop-production.up.railway.app/checkout/pro
...
- **stdio** (primary): `npx -y mcp-memory-gateway@0.8.5 serve` — version-pinned portable MCP launcher for Claude Code desktop and CLI

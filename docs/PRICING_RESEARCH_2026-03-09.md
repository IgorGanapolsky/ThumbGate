...
Keep Cloud Pro at the current live Stripe price of `$10/mo` for now, but position it explicitly as a founding price rather than the permanent list price.
...
- The open-source package is free, so Cloud Pro has to clear a second spending threshold on top of the user's existing model subscriptions.
...
- Keep `Cloud Pro` at `$10/mo` while proving onboarding, activation, and retention.

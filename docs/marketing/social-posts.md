# Social Launch Kit -- Claude Workflow Hardening

This is the current canonical social package for March 2026.

Use this kit instead of older memory-first launch copy when posting about the product on Reddit, LinkedIn, and X.

## Positioning

- Category: Claude workflow hardening for AI coding teams
- Wedge: one workflow, one owner, one proof pack
- Loop: feedback -> retrieval -> prevention rules -> verification
- Product promise: make one workflow safe enough to ship

## Canonical Message

> The sellable unit is not a generic AI employee. It is one workflow with memory, checks, and proof.

## Channel Packages

- LinkedIn long-form post: [linkedin-ai-reliability-post.md](./linkedin-ai-reliability-post.md)
- X thread: [x-launch-thread.md](./x-launch-thread.md)
- Reddit posts: [reddit-posts.md](./reddit-posts.md)
- Cursor plugin launch kit: [cursor-plugin-launch.md](./cursor-plugin-launch.md)
- Guarded template channel listings: [guarded-template-channel-listings.md](./guarded-template-channel-listings.md)
- Zero-filming IG + TikTok ops: [social-automation.md](./social-automation.md)

## Asset Inventory

Local, private-first export-ready assets live under [assets](./assets/README.md).

- [LinkedIn slide 1](./assets/ai-reliability-system-linkedin-slide-01.svg)
- [LinkedIn slide 2](./assets/ai-reliability-system-linkedin-slide-02.svg)
- [LinkedIn slide 3](./assets/ai-reliability-system-linkedin-slide-03.svg)
- [LinkedIn slide 4](./assets/ai-reliability-system-linkedin-slide-04.svg)
- [LinkedIn slide 5](./assets/ai-reliability-system-linkedin-slide-05.svg)
- [LinkedIn slide 6](./assets/ai-reliability-system-linkedin-slide-06.svg)
- [X summary card](./assets/ai-reliability-system-x-card.svg)
- [IG/TikTok carousel HTML](./assets/pre-action-checks-instagram-carousel.html)
- [IG/TikTok caption](./assets/pre-action-checks-caption.txt)

## Publishing Order

1. X thread
2. LinkedIn post plus first comment
3. Reddit text post

## Tracking Links

### LinkedIn

`https://thumbgate-production.up.railway.app/?utm_source=linkedin&utm_medium=organic_social&utm_campaign=claude_workflow_hardening_march_2026&utm_content=founder_post&campaign_variant=workflow_hardening_sprint&offer_code=LINKEDIN-SPRINT#workflow-sprint-intake`

### X

`https://thumbgate-production.up.railway.app/?utm_source=x&utm_medium=organic_social&utm_campaign=claude_workflow_hardening_march_2026&utm_content=thread&campaign_variant=workflow_hardening_sprint&offer_code=X-SPRINT#workflow-sprint-intake`

### Reddit

`https://thumbgate-production.up.railway.app/?utm_source=reddit&utm_medium=organic_social&utm_campaign=claude_workflow_hardening_march_2026&utm_content=claudecode_post&community=ClaudeCode&campaign_variant=workflow_hardening_sprint&offer_code=REDDIT-SPRINT#workflow-sprint-intake`

## Guardrails

- Lead with one workflow and one failure pattern, not infrastructure.
- Do not lead with pricing.
- Keep proof close to the pitch.
- Use one link only per post unless the channel requires a first comment.
- Keep the final assets local; do not treat shared Figma as the source of truth.

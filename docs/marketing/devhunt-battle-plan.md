# DevHunt Battle Plan — Tool of the Week

## Target: 25+ votes (last winner: Bugzy AI with 19)

## Submission Details

- **URL:** https://devhunt.org → Login → Submit your Dev Tool
- **Auth:** GitHub OAuth (use IgorGanapolsky account)
- **Submit:** Monday 8-9am US Eastern (full 7-day window)
- **Categories:** AI, Open Source, Code, CLI, Workflow Automation

## Listing Copy

**Tool Name:** ThumbGate

**Tagline (51 chars):**
> One thumbs-down permanently blocks that AI mistake

**Description:**
> ThumbGate puts a gate before every AI agent action. One thumbs-down captures the mistake, generates a prevention rule, and permanently blocks that pattern from recurring — across Claude Code, Cursor, Codex, Gemini, Amp, and OpenCode. Free to start; Pro at $19/mo adds the personal dashboard and DPO export. Team at $99/seat/mo adds shared governance for engineering teams.

**Pricing:** Freemium (Free + Pro $19/mo + Team $99/seat/mo)

**URLs:**
- Homepage: https://thumbgate-production.up.railway.app
- GitHub: https://github.com/IgorGanapolsky/ThumbGate
- npm: https://www.npmjs.com/package/thumbgate

## Visual Assets Needed

1. **Square logo** (256x256 PNG) — ThumbGate logo with gate icon
2. **Hero banner** (1200x630) — "One thumbs-down. Permanently blocked." with before/after
3. **Screenshot 1:** Terminal showing `npx thumbgate init` + gate blocking output
4. **Screenshot 2:** Dashboard at /dashboard showing blocked mistakes
5. **Screenshot 3:** Before/after diagram (from README)
6. **Screenshot 4:** Pricing page at /#pricing
7. **Video (optional but high-impact):** 60-second Loom demo

## Vote Mobilization Plan (Target: 25 votes in 7 days)

### Day 0 (Monday — Submit Day)

**Morning (8-9am ET):**
- Submit listing on DevHunt
- Tweet: "ThumbGate is live on @devhunt_ this week. Pre-action gates for Claude Code, Cursor & Codex. One 👎 = permanent block. Vote → [link] #claudecode #cursor #aicoding"
- Post on LinkedIn with same angle

**Midday:**
- Add temporary banner to GitHub README (top line):
  ```
  > 🏆 **[Vote for ThumbGate on DevHunt this week →](https://devhunt.org/tool/thumbgate)**
  ```
- Pin a GitHub Discussion: "ThumbGate is on DevHunt — vote takes 10 seconds"

### Day 1-2 (Tue-Wed — Community Push)

**Reddit posts (value-first, not salesy):**
- r/ClaudeCode: "I built gates that permanently block Claude Code from repeating your mistakes. Just launched on DevHunt."
- r/cursor: "If your Cursor agent keeps force-pushing to main, ThumbGate gates prevent that permanently."
- r/LocalLLaMA: "Pre-action gates for any MCP-compatible AI agent. Open source, local-first."

**Dev.to article:**
- Quick post: "How I Got My AI Agent to Stop Repeating the Same Mistake (and launched on DevHunt)"

**Direct outreach:**
- DM 10-15 Claude Code / Cursor power users on Twitter who've complained about agent mistakes
- Reply to relevant tweets about AI agent reliability

### Day 3-4 (Thu-Fri — Amplification)

- Comment on other DevHunt tools (genuine, helpful — drives profile views)
- Cross-post on Hacker News as "Show HN" if Reddit got traction
- Reply to DevHunt's official tweet about the weekly competition

### Day 5-7 (Sat-Sun — Final Push)

- Reminder tweet: "Last chance to vote for ThumbGate on DevHunt. 24 hours left."
- Email list (if any subscribers): "We need 5 more votes to win Tool of the Week"

## Competitive Intelligence

| Competitor | Votes | Threat | Our Advantage |
|---|---|---|---|
| Bugzy AI | 19 (last week) | Low — QA space | Different category (agent safety vs QA) |
| Concordance | 0 | None | We have npm traction they don't |
| Knight AI | 1 | None | We prevent mistakes, they automate tickets |
| Claudify | 28 (2 weeks ago) | Same audience | Complementary — their voters are our voters |

## Why We Win

1. **724 weekly npm installs** — real users to mobilize (3-4% conversion = 22-29 votes)
2. **Unique mechanism** — no one else has PreToolUse gates + Thompson Sampling
3. **6 agent platforms** — broadest compatibility (Claude Code, Cursor, Codex, Gemini, Amp, OpenCode)
4. **Visual before/after** — the README diagram is immediately understandable
5. **Claudify's audience** — Claude Code users who voted for Claudify 2 weeks ago are our ideal voters

## Post-Win Benefits

- Winner badge on DevHunt profile
- Featured in weekly email to all DevHunt subscribers
- Social media mention from @devhunt_
- SEO backlink from devhunt.org
- Credibility for Product Hunt launch (can reference "DevHunt Tool of the Week")

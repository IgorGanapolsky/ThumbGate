...
*   **Pre-Action Checks:** Think of this as the "content moderation" for an agent's internal brain. Before an agent can act on a retrieved memory, our **Pre-Action Checks** check it against a set of prevention rules derived from every past failure.

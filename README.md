...
**Self-Improving Firewall for AI Agents.** AI coding agents repeat mistakes — and one wrong tool call can wipe a directory, leak a key, or push broken code.
...
Accepted feedback is stored as local lessons. Repeated concrete failures can become prevention rules that flag or block matching tool calls according to policy. Lessons are promoted or demoted and re-ranked over time so the firewall improves from operations — without retraining the model.

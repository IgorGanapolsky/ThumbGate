...
- `npm run pr:manage` is the queue conductor. It must suppress duplicate `/trunk merge` submissions while a draft `trunk-merge/pr-*` shadow PR exists, prioritize release and workflow lanes ahead of dependency and feature lanes, and enforce queue backpressure instead of flooding `main`.

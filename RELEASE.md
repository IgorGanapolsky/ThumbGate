...
8. Verify the published package is live: `npm view thumbgate version`.
9. Refresh Cursor plugin metadata when marketplace copy, screenshots, or README changed.
10. Update `CHANGELOG.md`.

Cursor-specific rule: the plugin runtime uses `npx -y thumbgate@latest serve`, so npm releases can flow into the installed plugin runtime. Marketplace metadata and Cursor Directory copy do not auto-refresh from npm and require an explicit plugin/update pass. See [docs/CURSOR_PLUGIN_OPERATIONS.md](docs/CURSOR_PLUGIN_OPERATIONS.md).

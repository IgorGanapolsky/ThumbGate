...
Add a local AI-identity checklist doctor and an always-fused knowledge-graph retrieval pipeline.
...
Successor of #3651: rebased onto current `main` without the stale `npm test` chain. Ablation already fail-closes on 6 cases / 95% recall / 15% precision / 100% per-case recall; malformed `validFrom`/`validTo` fail closed. Scripts are not packed (same as the original PR).

...
function buildRevenueLinks(
  config = resolveHostedBillingConfig({
...
function buildMotionCatalog(links = buildRevenueLinks()) {
  return {
    pro: {
      ...
      audience:
        "Solo builders and small teams who need synced memory, gates, and usage analytics without a services engagement.",
    ...
    sprint: {
      ...
      audience:
        "Teams with one production workflow, one owner, and one repeated failure pattern blocking rollout.",
...
function summarizeCommercialSnapshot(summary = {}) {
  ...
  const qualifiedWorkflowSprintLeads =
    pipeline.qualifiedWorkflowSprintLeads || {};
...
function deriveRevenueDirective(
  ...
  if (snapshot.paidOrders > 0 || snapshot.bookedRevenueCents > 0) {
    return {
      ...
      objective:
        "Scale the first-10-customers loop with proof-backed self-serve follow-up.",
      primaryMotion: motionCatalog.pro.key,
      secondaryMotion: motionCatalog.sprint.key,
      headline:
        "Revenue is proven. Double down on the self-serve Pro CTA and use the sprint motion for expansion deals.",
      actions: [
  ...
  if (
    ...
    return {
      ...
      objective:
        "Convert existing interest into the first paid orders without inventing traction.",
      primaryMotion: motionCatalog.pro.key,
      secondaryMotion: motionCatalog.sprint.key,
      headline:
        "Interest exists but paid conversion is still zero. Push the $19/mo Pro CTA to builders and reserve sprint outreach for team workflows.",
      actions: [
  ...
  return {
    state: "cold-start",
    objective:
      "Land the first 10 paying customers with a founder-led, proof-backed dual motion.",
    primaryMotion: motionCatalog.pro.key,
    secondaryMotion: motionCatalog.sprint.key,
    headline:
      "No verified revenue and no active pipeline. Run dual motion: Pro for individual builders, sprint for teams with one workflow problem.",
    actions: [
      "Lead builder outreach with Pro at $19/mo and the direct checkout link.",
  ...
  for (const endpoint of queries) {
    ...
    for (const repo of items.slice(0, maxTargets * 2)) {
      combined.push({
        ...
        description:
          normalizeText(repo.description) || "No description provided.",
...
function selectOutreachMotion(target, motionCatalog = buildMotionCatalog()) {
  const haystack =
    `${normalizeText(target.repoName)} ${normalizeText(target.description)}`.toLowerCase();
  const sprintSignals =
    /(platform|workflow|ops|compliance|audit|enterprise|production|reliability|rollout|incident|governance)/;
  if (sprintSignals.test(haystack)) {
    return {
      key: motionCatalog.sprint.key,
      label: motionCatalog.sprint.label,
      reason:
        "Target language suggests a team workflow or production rollout problem.",
    };
  }
  ...
  return {
    ...
    reason:
      "Target looks builder-led, so the self-serve Pro CTA is the lowest-friction path.",
  };
}
...
function buildRevenueLoopReport({
  source,
  fallbackReason,
  summary,
  motionCatalog,
...
function renderRevenueLoopMarkdown(report) {
  ...
  lines.push(
    `- Billing source: ${report.source}${report.fallbackReason ? ` (${report.fallbackReason})` : ""}`,
  );
  lines.push("");
...
function writeRevenueLoopOutputs(report, options = {}) {
  ...
  if (reportDir) {
    ensureDir(reportDir);
    fs.writeFileSync(
      path.join(reportDir, "gtm-revenue-loop.md"),
      markdown,
      "utf8",
    );
    fs.writeFileSync(
      path.join(reportDir, "gtm-revenue-loop.json"),
...
async function runRevenueLoop(options = {}) {
  ...
  const { source, summary, fallbackReason } =
    await getOperationalBillingSummary();
  const directive = deriveRevenueDirective(summary, motionCatalog);
  const { targets, errors } = prospectTargets(options.maxTargets || 6);
  const enrichedTargets = await generateOutreachMessages(
    targets,
    motionCatalog,
  );

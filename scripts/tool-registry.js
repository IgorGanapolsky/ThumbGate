  ...
  readOnlyTool({
    ...
    inputSchema: {
      ...
      properties: {
        ...
        tenantId: { type: 'string', description: 'Hard tenant boundary; defaults to local.' },
        projectId: { type: 'string', description: 'Optional hard project boundary.' },
        entityId: { type: 'string', description: 'Optional hard entity boundary.' },
        visibility: { type: 'string', enum: ['private', 'shared', 'public'], description: 'Optional hard visibility filter.' },
        conversationContext: { type: 'string', description: 'Optional prior-turn context used only for selective ambiguous-query rewriting.' },
      },
    },
  }),
  readOnlyTool({
    name: 'rag_operations',
    description: 'Inspect production RAG stage contracts, recent stage health, document version and indexing state, and vector-index availability without exposing raw queries.',
    inputSchema: {
      type: 'object',
      properties: {
        telemetryLimit: {
  ...
  destructiveTool({
    ...
    inputSchema: {
      ...
      properties: {
        ...
        sourceFormat: { type: 'string', enum: ['markdown', 'text', 'yaml', 'json', 'html', 'pdf', 'docx', 'image'], description: 'Optional source format override. Binary formats require filePath.' },
        ...
        tenantId: { type: 'string', description: 'Required access boundary when importing tenant-scoped content; defaults to local.' },
        projectId: { type: 'string', description: 'Optional project boundary used as a hard retrieval filter.' },
        entityId: { type: 'string', description: 'Optional entity boundary used as a hard retrieval filter.' },
        visibility: { type: 'string', enum: ['private', 'shared', 'public'], description: 'Retrieval visibility; defaults to private.' },
        trustLevel: { type: 'string', enum: ['trusted', 'untrusted'], description: 'Source authority. Imported content defaults to untrusted.' },
        author: { type: 'string', description: 'Optional source author metadata.' },
        publishedAt: { type: 'string', description: 'Optional source publication timestamp.' },
        language: { type: 'string', description: 'Optional BCP-47 language label.' },
      },
    },
  }),
  readOnlyTool({
  ...
  readOnlyTool({
    ...
    inputSchema: {
      ...
      properties: {
        ...
        projectId: { type: 'string', description: 'Optional hard project boundary.' },
        entityId: { type: 'string', description: 'Optional hard entity boundary.' },
        visibility: { type: 'string', enum: ['private', 'shared', 'public'], description: 'Optional visibility filter.' },
        conversationContext: { type: 'string', description: 'Optional prior-turn context for selective ambiguous-query rewriting.' },
      },
    },
  }),
  readOnlyTool({

...
/**
 * Deterministic seeded-eval pipeline for ThumbGate RAG.
 *
 * Production document ingestion lives in document-intake.js and
 * document-parser.js. This synchronous adapter only builds stable text fixtures
 * for release evaluation; binary inputs explicitly route callers to the async
 * production parser rather than pretending to parse bytes.
 */
...
function parseDocument(input = {}) {
  ...
  if (
    ...
    || type === 'docx'
    || type === 'image'
    || /\.(?:pdf|docx|png|jpe?g|tiff?|bmp|webp)$/i.test(input.path || input.id || '')
  ) {
    return {
      ok: false,
      records: [],
      errors: [{
        ...
        message: 'Binary documents must use importDocumentAsync/document-parser for bounded parsing and OCR.',
      }],
    };
  }

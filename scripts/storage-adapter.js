...
// The local brain-memory backend lives in scripts/brain.js (a separate feature
// shipping on its own track). Load it optionally so the storage adapter — and
// the core lesson/feedback path — work even when brain.js isn't present. Only
// the local-mode brain-memory methods need it; postgres mode uses postgres-db.
const { loadOptionalModule } = require('./private-core-boundary');
function localBrain() {
  return loadOptionalModule('./brain', () => {
    throw new Error('local brain backend (scripts/brain.js) is not available in this build');
  });
}
...
async function upsertBrainMemory(entry) {
  if (STORAGE_MODE === 'postgres') {
    ...
    const result = localBrain().recordMemory(process.cwd(), entry);
...
async function searchBrainMemorySimilar(queryText, limit = 5, type = null) {
  if (STORAGE_MODE === 'postgres') {
    ...
    return localBrain().scanMemoryFiles(process.cwd())

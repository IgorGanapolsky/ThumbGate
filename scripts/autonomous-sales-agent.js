...
async function main(argv = process.argv.slice(2), overrides = {}) {
  ...
  const openCodePack = deps.buildOpenCodeRevenuePack(report);
  const openCodeWritten = deps.writeOpenCodeRevenuePack(openCodePack, options);

...
const CLI_COMMANDS = [
  ...
  },
  {
    ...
    flags: [
      jsonFlag(),
      { name: 'task', type: 'string', description: 'Task description for routed context packs' },
      { name: 'type', type: 'string', description: 'Memory type: decision | pattern | feedback | log' },
      { name: 'title', type: 'string', description: 'Memory title' },
      { name: 'content', type: 'string', description: 'Memory body' },
      { name: 'source', type: 'string', description: 'Required source/provenance for memory writes' },
      { name: 'tags', type: 'string', description: 'Comma-separated memory tags' },
      { name: 'text', type: 'string', description: 'Text or action to check against never-do gates' },
      { name: 'stale-days', type: 'number', description: 'Age threshold for cleanup reports' },
    ],
  },
  {
    name: 'stats',
  ...
  },
  {
    ...
    aliases: ['migrate-to-pg'],
    description: 'Generate or apply local JSONL import SQL for Team/Enterprise Postgres',
    group: 'advanced',
    flags: [
      jsonFlag(),
      { name: 'feedback-dir', type: 'string', description: 'Directory containing feedback-log.jsonl and memory-log.jsonl' },
      ...
      { name: 'agent-id', type: 'string', description: 'Imported agent id' },
      { name: 'out', type: 'string', description: 'Write generated SQL to this path' },
      { name: 'apply', type: 'boolean', description: 'Apply SQL using DATABASE_URL' },
    ],
  },
  {
    name: 'setup-postgres',
    description: 'Generate or apply Team/Enterprise Postgres + pgvector schema',
    group: 'advanced',
    flags: [
      jsonFlag(),
      ...
      { name: 'out', type: 'string', description: 'Write generated SQL to this path' },
      { name: 'apply', type: 'boolean', description: 'Apply SQL using DATABASE_URL' },
      { name: 'no-rls', type: 'boolean', description: 'Disable generated Row Level Security policies' },
    ],
  },
  {
    name: 'enterprise-postgres',
    description: 'Advanced Team/Enterprise Postgres schema and migration wrapper',
    group: 'advanced',
    flags: [
      jsonFlag(),
      { name: 'embedding-dim', type: 'number', description: 'pgvector embedding dimension (default 768)' },
      { name: 'feedback-dir', type: 'string', description: 'Directory containing feedback-log.jsonl and memory-log.jsonl' },
      { name: 'org-id', type: 'string', description: 'Enterprise organization id for migration' },
      { name: 'project-id', type: 'string', description: 'Enterprise project id for migration' },
      { name: 'out', type: 'string', description: 'Write generated SQL to this path' },
      { name: 'apply', type: 'boolean', description: 'Apply SQL using DATABASE_URL' },
    ],
  },

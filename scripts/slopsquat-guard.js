...
const NPM_INSTALL = /\b(?:npm|pnpm|yarn|bun)\s+(?:install|i|add|in)\b/;
const NPM_EXEC = /\b(?:npx|pnpm\s+dlx|bunx|yarn\s+dlx)\s+/;
const PIP_INSTALL = /\b(?:pip3?|uv\s+pip|pipx)\s+install\b|\b(?:uv|poetry)\s+add\b|\bpython3?\s+-m\s+pip\s+install\b/;
...
function isNonPackageToken(tok) {
  ...
  // Common shell commands to ignore if they appear in segments
  const SHELL_CMDS = new Set(['echo', 'cat', 'ls', 'cd', 'sudo', 'time', 'env', 'xargs', 'mkdir', 'rm', 'cp', 'mv']);
  if (SHELL_CMDS.has(tok.toLowerCase())) return true;
  return false;
}
...
function parseInstallCommands(command) {
  ...
  // 1. Handle backslash line continuation
  const joined = command.replace(/\\\n/g, ' ');

  // 2. Split into shell segments, merging piped xargs
  const rawSegments = joined.split(/&&|\|\||;|\n|\|/);
  const segments = [];
  for (let i = 0; i < rawSegments.length; i++) {
    let s = rawSegments[i];
    while (i + 1 < rawSegments.length && rawSegments[i + 1].trim().startsWith('xargs')) {
      s += ' ' + rawSegments[i + 1];
      i++;
    }
    segments.push(s);
  ...
  // Leading command prefixes / wrappers. Split into simple, non-backtracking
  // alternatives (no nested quantifiers) to avoid super-linear regex runtime
  // (ReDoS / SonarCloud S5852). The loop below strips one occurrence per pass,
  // so `env A=1 B=2 npm i` is peeled across iterations.
  const PREFIX_WORD = /^(?:sudo|time|xargs|env)\s+/;   // wrapper command words
  const ENV_ASSIGNMENT = /^[\w.-]+=\S+\s+/;            // a single VAR=value
  const WRAPPER_OPEN = /^[({]\s*/;                      // subshell / group open

  for (let seg of segments) {
    seg = seg.trim();
    if (!seg) continue;

    // 3. Strip wrappers and prefixes iteratively
    let changed = true;
    while (changed) {
      const before = seg;
      // Strip leading prefixes (one of each per pass; loop repeats until stable)
      seg = seg
        .replace(PREFIX_WORD, '')
        .replace(ENV_ASSIGNMENT, '')
        .replace(WRAPPER_OPEN, '')
        .trim();
      // Strip trailing subshell/group closers
      seg = seg.replace(/\s*[)}]$/, '').trim();
      if (seg === before) changed = false;
    }
    ...
    if (NPM_EXEC.test(seg)) {
      ecosystem = 'npm';
      isExec = true;
    } else if (NPM_INSTALL.test(seg)) {
      ecosystem = 'npm';
    ...
    let foundExecTarget = false;

    for (const tok of tokens) {
      if (isFlag(tok)) continue;
      if (isNonPackageToken(tok)) continue;

      // Skip the verbs themselves
      if (tok === 'install' || tok === 'i' || tok === 'add' || tok === 'in' || 
          tok === 'pip' || tok === 'pip3' || tok === 'npm' || tok === 'yarn' || 
          tok === 'pnpm' || tok === 'bun' || tok === 'npx' || 
          tok === 'python' || tok === 'python3' || tok === 'uv' || tok === 'poetry') continue;
...
// Build a registry URL with each path segment encoded independently. Avoids
// fragile post-hoc unescaping (a scoped name "@scope/pkg" needs the '@' literal
// and the '/' as %2f) and keeps every component fully sanitized.

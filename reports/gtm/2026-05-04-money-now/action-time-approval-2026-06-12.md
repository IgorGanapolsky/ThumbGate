...
- Priority: A2
- Goal: create the first visible value-first Skool post
- Copy source:
  - `reports/gtm/2026-05-04-community-course-promo/skool-public-post-draft-2026-06-11.md`
- Current evidence:
  - headless Skool readback re-verified at `2026-06-12T18:05:34Z` and still shows `1` member with `0` visible posts
  - Discovery guidance still calls for at least one post plus invited members/activity for newer groups
  - this is the lowest-risk community unlock because it is copy-only and does not depend on the missing local asset folder
- Confirmation needed because: a live Skool post changes third-party state

## Action 3

- Type: free Classroom starter-course edit
- Priority: A3
- Goal: create the first free starter course surface
- Copy sources:
  - `reports/gtm/2026-05-04-community-course-promo/skool-classroom-listing-copy.md`
- Current evidence:
  - headless Skool readback re-verified at `2026-06-12T18:05:34Z` and shows the public surface is still sparse: `1` member, `0` visible posts
  - Classroom docs still support `Open` access, published-by-default courses, native upload, and Loom/video embed fallback
  - local media assets referenced by older notes are still missing from this checkout
  - official Skool docs confirm both native video upload and Loom embeds, but the current local environment is still blocked on the native file picker
...
- Current evidence:
  - local dry-run re-verified at `2026-06-12T18:05:34Z` and returns `6` previews plus `0` errors
  - asset resolution still points at missing `docs/marketing/assets/*` files
  ...
  - `npm run social:zernio:status` re-verified at `2026-06-12T18:05:22.035Z` still reports `0/6` healthy platforms and `0` rows in the last `24h`
...
2. `Approve A2 first public Skool post`
3. `Approve A2 copy-only first public Skool post`
4. `Approve A3 free Classroom starter-course edit`
5. `Approve A3 copy-only free Classroom starter-course edit`
6. `Approve A3 Loom-backed free Classroom starter-course edit`
7. `Approve A4 Operator Lab promo preview`
8. `Approve A4 Operator Lab promo schedule for <timestamp>`
9. `Approve A4 Operator Lab promo publish now`

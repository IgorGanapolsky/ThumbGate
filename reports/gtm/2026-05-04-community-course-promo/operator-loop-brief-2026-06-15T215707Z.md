# Operator Loop Brief — 2026-06-15T21:57:07Z
...
- `npm run sales:pipeline -- summary` at `2026-06-15T21:56:35Z` still shows `24` active leads with actionable `byStage` truth of `22` `contacted`, `2` `replied`, `0` `checkout_started`, `0` `sprint_intake`, and `0` `paid`; the script still overstates the top-level `contacted` field as `24`.
- `node scripts/skool-reader.js --url https://www.skool.com/thumbgate-operator-lab-6000 --limit 10 --format markdown` at `2026-06-15T21:56:36Z` still shows `Members: 1` and `Visible posts on page: 0`.
- `npm run social:zernio:status` at `2026-06-15T21:56:18Z` still shows `0/6` healthy platforms, `0` rows in the last `24h`, and exits non-zero with the same likely-cause guidance.
- `npm run social:publish:launch -- --dry-run --offer=operator-lab --platforms=linkedin,instagram,threads,bluesky,reddit,youtube` at `2026-06-15T21:56:35Z` still renders `6` previews, `0` errors, committed media under `public/assets/brand/*`, and `accountCount: 0` on every platform.

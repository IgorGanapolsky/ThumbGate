# Operator Loop Brief — 2026-06-15T05:45:53Z
...
- No material requirement delta surfaced versus the prior refresh.
- Important wording to preserve in operator notes:
  - newer Discovery FAQ says listing appears within about `two hours` after threshold is met
  - older troubleshooting article still says `within an hour`
  - prefer the newer FAQ wording
- Membership questions still cap at `3`, with only `1` email-answer question allowed.
- Classroom remains the core course/resource surface, and About remains the landing/checkout surface.

## Revenue read

- Fastest money path is still the warm Reddit follow-up four-pack already staged in `reports/gtm/2026-05-04-money-now/operator-send-now.md`.

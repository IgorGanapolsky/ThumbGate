# Operator Loop Brief — 2026-06-14T18:41:19Z
...
- `node scripts/sales-pipeline.js summary` at `2026-06-14T18:40:48Z` still shows `24` active leads with actionable `byStage` truth of `22` `contacted`, `2` `replied`, `0` `checkout_started`, `0` `sprint_intake`, and `0` `paid`.
...
- `node scripts/skool-reader.js --url https://www.skool.com/thumbgate-operator-lab-6000 --limit 10 --format markdown` at `2026-06-14T18:40:48Z` still returns only `Members: 1` and `Visible posts on page: 0`.
- `npm run social:publish:launch -- --dry-run --offer=operator-lab --platforms=linkedin,instagram,threads,bluesky,reddit,youtube` at `2026-06-14T18:40:48Z` still returns `6` previews, `0` errors, `0` published, and `0` scheduled.
...
- `npm run social:zernio:status` at `2026-06-14T18:40:48Z` still reports `0/6` healthy platforms and `0` rows in the last `24h`.
- Local secret readback at `2026-06-14T18:40:48Z` still found no `ZERNIO_API_KEY` in env and no `ZERNIO_API_KEY=` line in local `.env` or `.env.example`.
...
- Discovery FAQ still says visibility lands within about `two hours` after threshold, which should override older `within an hour` wording.
...
- About page remains required for Discovery, and Classroom still has to be enabled before a starter course is truly visible.

...
- `node scripts/sales-pipeline.js summary` on `2026-06-14` still shows `24` active leads with actionable `byStage` truth of `22` `contacted`, `2` `replied`, `0` `checkout_started`, `0` `sprint_intake`, and `0` `paid`.
- `npm run social:publish:launch -- --dry-run --offer=operator-lab --platforms=linkedin,instagram,threads,bluesky,reddit,youtube` on `2026-06-14` still returns `6` previews, `0` errors, `0` published, and `0` scheduled.
...
- `npm run social:zernio:status` at `2026-06-14T23:43:00Z` is still dark: `0/6` healthy platforms and `0` rows in the last `24h`.
- Local secret readback on `2026-06-14` still shows `ZERNIO_API_KEY` absent from env, `.env`, and `.env.example`.
- `node scripts/skool-reader.js --url https://www.skool.com/thumbgate-operator-lab-6000 --limit 10 --format markdown` on `2026-06-14` still returns `Members: 1` and `Visible posts on page: 0`.
...
- The current Discovery FAQ still says visibility usually lands within about `two hours` after threshold.
- The older discovery troubleshooting article still says `within an hour`; keep preferring the newer FAQ wording.
- Discovery ranking still penalizes `off-platform payments`, so public Skool copy should remain value-first.
...
- Account security guidance still prohibits sharing creator credentials, codes, or cookies.
...
## Course/community copy delta

- The repo checkout does not currently contain the Skool asset pack described in the original 2026-05-05 state under `docs/marketing/assets/`.
- The only locally verified promo media in this checkout are the committed fallback brand assets under `public/assets/brand/*`.
- Until authenticated Skool access is approved, treat the existing course/about copy as content-ready but asset-source-unverified.
- If action-time confirmation is granted for a live Skool edit, verify the real upload source before attempting cover, icon, About, or explainer-video updates.

# Acquisition Queue (2026-05-06) — Operator Lab → Diagnostic/Sprint
...
Goal: convert warm + high-intent operators into:
...
Canonical queue + logging commands:
...
2. **Reliability Gateway**: feedback → memory → prevention rule → enforcement (not vibes).
3. **Proof pack**: before/after behavior + verification evidence (no ROI claims).
4. **Thompson Sampling** lessons: reduce repeated agent mistakes without brittle prompt hacks.

...
function safetyCitationGate(answer, route) {
  ...
  const cited =
    ...
    /\[(Safety Procedures Manual|Maintenance Manual|Quality Control Standards|Quality Standards Manual|Protocol Specification|Live PLC Telemetry|OSHA\s*\d*),\s*(p(g|age)?\.\s*\d+|p\.\s*\d+|[Pp]age\s*\d+|[Ll]ive)\]/i.test(answer) ||
    /\b(Safety Procedures Manual|Maintenance Manual|Quality Control Standards|Quality Standards Manual|Protocol Specification|Live PLC Telemetry)\b[\s\S]{0,250}\b(p(g|age)?\.\s*\d+|p\.\s*\d+|[Pp]age\s*\d+|[Ll]ive)/i.test(answer);
  return {
